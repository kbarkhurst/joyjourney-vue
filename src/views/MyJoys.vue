<template>
  <main>
    ...
    <!-- If Page is NOT YOURS -->
    <div v-if="url_route != username">
      <div id="topbar" class="container-fluid sticky-top position-fixed bgorangegrad offsetpaddingmargin pb-4">
        <div class="w-75 text-center pt-5">
          <div class="row">
            <div class="col-sm-5">
              <h1>{{ url_route }}'s Joys</h1>
              <router-link
                v-if="userdata.journal_description"
                title="Profile"
                v-bind:to="{ path: '/' + url_route + '/journal/' }"
              >
                Profile
              </router-link>
            </div>
...
export default {
  data: function () {
    return {
      ...
      years: [],
      yearSelected: null,
      url_route: this.$route.params.username,
      userdata: [],
    };
  },
  // mounted() {
  ...
  watch: {
    ...
    userName(newValue, oldValue) {
      if (newValue !== oldValue) this.isNewUsername = true;
      this.setUsername()
        .then(() => {
